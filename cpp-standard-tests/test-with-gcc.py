...
def run_gcc_on_file(file_path, std_flag):
    """Run g++ on the given C++ file and return the result."""
    try:
        result = subprocess.run(['g++', std_flag, '-o', '/dev/null', file_path], capture_output=True, text=True)
...
def main():
    ...
    for cpp_file in ordered_cpp_files:
        ...
        print(f"Running g++ {std_flag} on {cpp_path}...")
        stdout, stderr, returncode = run_gcc_on_file(cpp_path, std_flag)
        results[cpp_file] = {
            'stdout': stdout,
            'stderr': stderr,

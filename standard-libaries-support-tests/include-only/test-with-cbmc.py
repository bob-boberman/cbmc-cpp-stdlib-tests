...
def extract_group_from_path(path):
    # Returns the first subfolder under src/, or "." if none
...
def main():
    ...
    for cpp_file in cpp_files:
        ...
        std_flag = CBMC_STANDARD_FLAGS.get(std, None)
        retcode, out, err = run_cbmc_on_file(cpp_file, std_flag)
        if retcode == 0:
            result = "PASS"
        elif retcode == -999:
            result = "TIMEOUT"
    ...
    print(f"{'File':<60} | {'Result':<10} | {'CBMC Return Code'} | {'CBMC Standard Flag'} | {'Group'}")
    print("-" * 120)
    ...
    # Custom order for groups: put c_header and c_wrapper at the top
    ...
    for group in sorted(results_by_group, key=group_sort_key):
        # Collect per-standard stats for this group (excluding subgroups)
        ...
        # Calculate group totals including subgroups
        ...
        # Print subgroups summary (if any), in desired order
        ...
        # Print any other subgroups not in the order list

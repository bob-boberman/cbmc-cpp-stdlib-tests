...
STANDARD_FLAGS = {
    ...
    # "cpp20": "-std=c++20",
    # "cpp23": "-std=c++23",
    "default": "-std=c++23",  # fallback for unknowns
# using the default for all improved the resutls a lot, there might be flags set not correctly, difference was about below 5% 
}
...
def run_gcc_on_file(file_path, std_flag):
    try:
        cmd = ["g++"]
        if std_flag:
            cmd.append(std_flag)
...
def main():
    ...
    for cpp_file in cpp_files:
        ...
        retcode, out, err = run_gcc_on_file(cpp_file, std_flag)
        if retcode == 0:
            result = "PASS"
        elif retcode == -999:
            result = "TIMEOUT"
    ...
    print(f"{'File':<60} | {'Result':<10} | {'g++ Return Code'} | {'Standard Flag'} | {'Group'}")
    print("-" * 120)

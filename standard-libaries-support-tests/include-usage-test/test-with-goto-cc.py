...
def run_goto_cc_on_file(file_path, std_flag):
    try:
        cmd = ["goto-cc"]
        if std_flag:
            cmd.append(std_flag)
...
def main():
    ...
    for cpp_file in cpp_files:
        ...
        retcode, out, err = run_goto_cc_on_file(cpp_file, std_flag)
        if retcode == 0:
            result = "PASS"
        elif retcode == -999:
            result = "TIMEOUT"
    ...
    print(f"{'File':<60} | {'Result':<10} | {'goto-cc Return Code'} | {'Standard Flag'} | {'Group'}")
    print("-" * 120)
